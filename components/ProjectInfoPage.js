...
const ProjectInfoPage = ({ projectName }) => {
  ...
  const generateTags = () => {
    const tagArray = itemData.tags.map((tag) => (
      <SkillTag key={uniqid()} tag={tag} />
    ));
              ...
              <Box key={uniqid()}>
                <SwiperSlide style={{ padding: "2px 0" }}>
                  <Box
                    ...
                    h="100%"
                    display="flex"
                    boxShadow="0 1px 6px #171717, 0 3px 6px #171717;"
                    bgColor="#030303"
                    borderRadius="10px"
                  >
                    <Box
